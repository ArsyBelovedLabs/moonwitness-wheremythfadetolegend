...
function render(report, issues, evidence) {
  document.querySelector('#kpis').innerHTML = report.kpis.map(k => `
    <article class="kpi"><div class="label">${esc(k.label)}</div><div class="value">${esc(k.value)}</div><div class="note">${esc(k.note)}</div></article>
  `).join('');

  renderObservations(report.observations);

  document.querySelector('#gapBars').innerHTML = report.gap_distribution.map(g => `
    <div class="bar-row"><div class="bar-head"><span>${esc(g.label)}</span><strong>${g.value}%</strong></div><div class="bar-track"><div class="bar-fill ${tone(g.value)}" style="width:${Math.min(100, Math.max(0, g.value))}%"></div></div></div>
  `).join('');
  ...
  document.querySelector('#evidenceCards').innerHTML = evidence.map(e => `
    <article class="evidence-card"><div class="eyebrow">${esc(e.type)}</div><h3>${esc(e.title)}</h3><p>${esc(e.description)}</p><a href="${esc(e.url)}" target="_blank" rel="noreferrer noopener">Open evidence ↗</a></article>
  `).join('');
...
function renderObservations(observations) {
  ...
  tbody.innerHTML = observations.map(o => `
    <tr>
      <td>${esc(o.date)}</td>
      <td><strong>${esc(o.location)}</strong></td>
      <td>${esc(o.actor)}</td>
      <td><strong>${esc(o.practice)}</strong><div class="table-summary">${esc(o.summary)}</div></td>
...
function bindFilters() {
  ...
  const apply = () => {
    ...
    const filtered = reportCache.observations.filter(o => {
      ...
      const matchesGap = selected === 'all' || tone(o.tauhid_gap) === selected;
