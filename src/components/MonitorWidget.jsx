...
const classifySignal = (item) => {
  const text = `${item?.title || ""} ${item?.description || ""}`.toLowerCase()
  if (/worship|pray|prayer|offering|sacrifice|summon|invoke|amulet|divination|fortune|spirit|ancestor/.test(text)) return "RITUAL"
  if (/myth|legend|folklore|ghost|spirit|supernatural|sacred|astrology|zodiac/.test(text)) return "MYTHOS"
  return "SIGNAL"
}
...
export default function MonitorWidget() {
  ...
  const refresh = () => loadJson("data/monitor/latest.json").then((data) => { setSnapshot(data); setError("") }).catch((e) => setError(e.message))
  ...
  const items = useMemo(() => (snapshot?.items || []).map((item) => ({ ...item, category: classifySignal(item) })).slice(0, 8), [snapshot])
        ...
        {error ? (
          ...
          <>
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
              ...
              <div className="rounded-lg border bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Errors</div><div className="mt-1 text-xl font-bold">{snapshot?.errorCount ?? 0}</div></div>
            </div>
            ...
            {items.length === 0 ? (
              <div className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">No new signals in the latest snapshot.</div>
            ) : (
              <div className="space-y-2">
                {items.map((item) => (
                  <a key={item.guid || item.url} href={item.url} target="_blank" rel="noreferrer" className="block rounded-lg border p-3 transition-colors hover:bg-muted/40">
            ...
            <div className="flex items-center gap-2 rounded-lg bg-muted/30 p-3 text-xs text-muted-foreground"><Activity className="size-4" /> Automatic scans run every 6 hours. Signals remain unreviewed until evidence verification.</div>
          </>
        )}
      </CardContent>
    </Card>
